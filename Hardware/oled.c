 ...
 ***************************************************************************************************8*/
/**
 * @function: void OLED_DrawDot(uint8_t x,uint8_t y)
 * @description: 屏幕中任意位置画点函数
 * @param {uint8_t} x 待显示点的开始横坐标	x: 0~127，
 * @param {uint8_t} y 待显示点的开始纵坐标 y: 0~63，
 * @return {*}
 */
void OLED_DrawDot(uint8_t x,uint8_t y)
{
	OLED_WR_CMD(0xb0+(y/8));	//设置页地址（0~7）
	OLED_WR_CMD(((x&0xf0)>>4)|0x10); //设置显示位置—列高地址
	OLED_WR_CMD(x&0x0f);	//设置显示位置—列低地址
    OLED_WR_DATA(0x01<<(y%8));//设置一列中8个“灯”的亮灭
}

/**
 * @function: void OLED_Draw_XLine(uint8_t x,uint8_t y,int length)
 * @description: 绘制沿x轴直线的函数(注：还不太完善，每一页只能画一条直线）
 * @param {uint8_t} x 待显示沿x轴直线的开始横坐标	x: 0~127，
 * @param {uint8_t} y 待显示沿x轴直线的开始纵坐标 y: 0~63，
 * @param {int} length 沿x轴直线长度，length值需要自己计算一下，不要超出边界范围就行
 * @return {*}
 */
...
/**
 * @function: void OLED_Draw_YLine(uint8_t x,uint8_t y,int length)
 * @description: 绘制沿y轴直线的函数
 * @param {uint8_t} x 待显示沿y轴直线的开始横坐标	x: 0~127，
 * @param {uint8_t} y 待显示沿y轴直线的开始纵坐标 y: 0~63，
 * @param {int} length 沿y轴直线长度，length值需要自己计算一下，不要超出边界范围就行
 * @return {*}
 */
void OLED_Draw_YLine(uint8_t x,uint8_t y,int length)
{
	uint8_t i,j,k, Light_num=0;
	OLED_WR_CMD(0xb0+(y/8));	//设置页地址（0~7）
	OLED_WR_CMD(((x&0xf0)>>4)|0x10); //设置显示位置—列高地址
	OLED_WR_CMD(x&0x0f);	//设置显示位置—列低地址
	Light_num=0x01<<(y%8 );
	if(length<=(8-y%8))
		//绘制的线的长度小于等于一列中剩余长度
		//length==3时，y%8==2时--> 3<=6
	{
			for(i=1;i<length;i++) //length-y%8+1== 2
			{
				Light_num|=((0x01<<(y%8))<<i);
			}
			OLED_WR_DATA(Light_num);
	}
	else
		//绘制的线的长度大于一列中剩余长度
		//length==8时，y%8==2时--> 8>6
	{
		Light_num|=(0xff<<(y%8));
		OLED_WR_DATA(Light_num);
		//下面的两个for循环是将所绘制直线的剩余部分拆封成两段，第一个for循环是将某一页中的某一列全部点亮
		//第二个for循环是将所绘制直线的最后一部分(不够8个灯的部分)按照设置的length数点亮“灯”
		for(j=1;j<=(length-(8-y%8) )/8;j++) //这是第一个for.  8-y%8==6
		{
			OLED_WR_CMD(0xb0+(y/8)+j);
			OLED_WR_CMD(((x&0xf0)>>4)|0x10); //设置显示位置—列高地址
			OLED_WR_CMD(x&0x0f);	//设置显示位置—列低地址
			OLED_WR_DATA(0xff);
		}
		if(((length-(8-y%8) )%8)!=0)
		{
			...
			OLED_WR_CMD(x&0x0f);	//设置显示位置—列低地址
			Light_num=0x01;
			for(k=1;k<(length-8*(j-1)-(8-y%8));k++)//这是第二个for.
			{
				Light_num|=(0x01<<k);
			}
			OLED_WR_DATA(Light_num);
