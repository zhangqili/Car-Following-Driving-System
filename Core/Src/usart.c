...
void Uart1_SendStr(char*SendBuf)//??1???????
{
  while(*SendBuf)
  {
    ...
    HAL_UART_Transmit(&huart1,(uint8_t*)SendBuf,1,1000);//????????
    SendBuf++;
  }
}
#else
void Uart1_SendStr(char *SendBuf)     //??1???????
{
    while (*SendBuf) {
        while (LL_USART_IsActiveFlag_TC(USART1) != 1)
            ;     //??????????
        LL_USART_TransmitData8(USART1, (uint8_t) (*SendBuf & (uint8_t) 0xff)); //????????
        SendBuf++;
    }
}
#endif
...
void uart2_revice_my_data(void)
{
    ...
    /*
    uint8_t Res = LL_USART_ReceiveData8(USART3);
    if (flag != 1) {
        if (state == 0) {     //???????
            if (Res == 0xfc) {
                header = 0x00fc;
